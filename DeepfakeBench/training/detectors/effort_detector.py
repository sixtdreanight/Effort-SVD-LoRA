...
class SVDMainOnlyLinear(nn.Module):
    def __init__(self,in_features:int,out_features:int,r:int,bias:bool = True,init_weight:torch.Tensor = None,init_bias:torch.Tensor = None):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.r = min(r,min(in_features,out_features))
        ...
        if init_weight is not None:
            self.__init__svd(init_weight,init_bias)
            
    def __init__svd(self,weight:torch.Tensor,bias:torch.Tensor = None):
        W = weight.data.double()
        U,S,Vh = torch.linalg.svd(W,full_matrices=False)
        
        r = self.r
        U_r = U[:,:r].to(weight.dtype)
        S_r = S[:r].to(weight.dtype)
        Vh_r = Vh[:r,:].to(weight.dtype)
        
        weight_main = U_r @ torch.diag(S_r) @ Vh_r
        self.weight_main.copy_(weight_main)
        
        if self.bias is not None and bias is not None:
            self.bias.copy_(bias.data)
            
    def forward(self,x:torch.Tensor) -> torch.Tensor:
        x = x.to(self.weight_main.dtype)
        return F.linear(x,self.weight_main,self.bias)
        
class LoRALinear(nn.Module):
    def __init__(self,linear_layer:nn.Module,rank:int = 4,alpha:int = 16,dropout:float = 0.1):
        super().__init__()
        ...
        in_features = self.linear.weight_main.size(1)
        out_features = self.linear.weight_main.size(0)
